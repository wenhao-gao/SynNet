...
def nn_search(_e, _tree, _k=1):
    ...
    return dist[0], ind[0]  # TODO double check this
...
def set_embedding_fullbeam(z_target, state, _mol_embedding):
    """
...
def synthetic_tree_decoder_fullbeam(z_target,
    ...
    for i in range(max_step):
        ...
        # from ipdb import set_trace; set_trace(context=11)
        state = tree.get_state() # a set
        z_state = set_embedding_fullbeam(z_target, state, mol_fp)
